...
# (this is not the original version - adapted to run outside Blender
# and just dump a mesh)

import re, struct, StringIO, array
...
verts, inds = filesel_callback("Armadillo.ply")
print ("%d verts, %d inds" % (len(verts),len(inds)))

file = open("Armadillo.bin", "wb")
file.write(struct.pack("<II", len(verts), len(inds))) # header
for v in verts:
        file.write(struct.pack("<fff", v[0], v[1], v[2]))
array.array('I', inds).tofile(file)
